...
def run_glide_finetune_epoch(
            ...
            wandb_run.log(
                {
                    **log,
                    "iter": train_idx,
                    "samples": wandb.Image(sample_save_path, caption="UC Davis campus"),
                }
            )

            print(f"Saved sample {sample_save_path}")

        # Updating variables for loss record
        epoch_loss += accumulated_loss.item() / gradient_accumualation_steps
        total_training += 1

    log['epoch'] = epoch
    log['loss'] = epoch_loss / total_training
    ...
    return epoch_loss / total_training
...
f"Saved checkpoint {train_idx} to {checkpoints_dir}/glide-finetuned.pt"
