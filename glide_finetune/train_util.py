...
def save_model(
    ...
    prev_path = os.path.join(checkpoints_dir, f"glide-finetuned-{epoch-3}.pt")
    ...
    th.save(
        ...
        os.path.join(checkpoints_dir, f"glide-finetuned-{epoch}.pt"),
    )
    
    tqdm.write(
        f"Saved checkpoint {train_idx} to {checkpoints_dir}/glide-finetuned-{epoch}.pt"
    )
...
def pil_image_to_norm_tensor(pil_image):
    ...
    if not np_image.flags['WRITEABLE']:
        np_image = np.copy(np_image)
        
    return th.from_numpy(np_image).float().permute(2, 0, 1) / 127.5 - 1.0
